...
- replace ``1:length(A)`` with ``linearindices(A)``
- replace explicit allocations like ``Array{Int}(size(B))`` with ``similar(Array{Int}, indices(B))``
- if necessary, wrap functions with ``@safeindices``

These are described in more detail below.

Safety features: new errors and ``@safeindices``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
and ``size`` throw an error (by default) when passed an array with
...
Once the code has been modified (see below), if needed you can wrap
this function using the macro ``@safeindices``.  For example::

  @safeindices longer_than_5(A::AbstractArray) = length(A) > 5

would ensure that this call to ``length`` succeeds even if ``A`` has
non-1 indices.

In many cases, you may find that you eliminate ``size`` and ``length``
from functions by replacing them with ``indices`` and
``linearindices``, so ``@safeindices`` is not needed in every case.

You can read about how ``@safeindices`` works in the online help.
...
at the top of any function.  See also
:ref:`devdocs-extending-safeindices` below.
...
Note that ``@safeindices`` is unnecessary.

Allocating storage using generalizations of ``similar``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Storage is often allocated with ``Array{Int}(dims)`` or ``similar(A, args...)``.
...
``AbstractUnitRange``, ``ZeroTo``, where ``ZeroTo(n)`` is equivalent
to ``0:n``.

In general, you should probably *not* export ``ZeroTo`` from your
package: there may be other packages that implement their own
``ZeroTo``, and having multiple distinct ``ZeroTo`` types is (perhaps
counterintuitively) an advantage: ``ModuleA.ZeroTo`` indicates that
``similar`` should create a ``ModuleA.ZeroArray``, whereas
``ModuleB.ZeroTo`` indicates a ``ModuleB.ZeroArray`` type.  This
...
avoid the need to write your own ``ZeroTo`` type.
...
Once you have your ``AbstractUnitRange`` type, then specialize ``indices``::
  ...
  Base.indices(A::ZeroArray) = map(n->ZeroTo(n-1), A.size)
...
Given your custom ``ZeroTo`` type, then you should also add the following two specializations for ``similar``::

  function Base.similar(A::AbstractArray, T::Type, shape::Tuple{ZeroTo,Vararg{ZeroTo}})
      # body
  end

  function Base.similar(f::Union{Function,Type}, shape::Tuple{ZeroTo,Vararg{ZeroTo}})
      # body
  end
...
::
   Base.reshape(A::AbstractArray, shape::Tuple{ZeroTo,Vararg{ZeroTo}}) = ...

and you can ``reshape`` an array so that the result has custom indices.

.. _devdocs-extending-safeindices:

Extending ``@safeindices``
~~~~~~~~~~~~~~~~~~~~~~~~~~

You can "protect" functions besides ``size`` and ``length``. Let's say
you have some mission-critical function ``foo``::

  foo(x::AbstractArray) = 1

Now suppose that this may or may not be safe if ``x`` is an array with
unconventional indexing.  You can force users to inspect each call to
``foo`` by redefining it like this::

  foo(::Base.SafeIndices, x) = 1
  foo(::Base.UnsafeIndices, x) = error("oops")

  # Make default safety dependent on using "conventional" indices
  foo(x::AbstractArray) = foo(indices(x), x)
  foo{N}(::NTuple{N,Base.OneTo}, x) = foo(Base.SafeIndices(), x)   # safe for "conventional" array
  foo(::Tuple, x)                   = foo(Base.UnsafeIndices(), x) # unsafe for any non-1 array

Then you can make ``@safeindices`` protect calls to ``foo`` with the line::

  push!(Base.safeindices_functions, :foo)
...
adopters should be prepared to identify and/or fix bugs.
